      ...
      <div className="siga8-mapping-page">
        ...
        <section className="data-panel siga8-mapping-table-card">
          ...
          {loading ? <div className="empty-state">Memuat pemetaan peran...</div> : mappings.length === 0 ? <div className="empty-state">Belum ada pemetaan peran SIGA8.</div> : (
            <div className="siga8-mapping-table-wrapper"><table className="siga8-mapping-table"><thead><tr><th>Peran SIGA8</th><th>ID SIGA8</th><th>Peran KlikPresensi</th><th>ID Role</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
              {mappings.map((mapping) => <tr key={mapping.id || mapping.siga8RoleId}><td><strong>{mapping.siga8RoleName}</strong></td><td><code>{mapping.siga8RoleId || "-"}</code></td><td><span className="mapping-local-role">{mapping.localRole}</span></td><td><input className="mapping-role-id" type="number" min="1" defaultValue={mapping.roleId} onBlur={(e) => { const roleId = Number(e.target.value); if (Number.isInteger(roleId) && roleId > 0 && roleId !== Number(mapping.roleId)) updateMapping(mapping, { role_id: roleId }); }} disabled={saving} aria-label={`ID role untuk ${mapping.siga8RoleName}`} /></td><td><button type="button" className={`mapping-status ${mapping.active ? "active" : ""}`} disabled={saving} onClick={() => updateMapping(mapping, { is_active: !mapping.active })}>{mapping.active ? "Aktif" : "Nonaktif"}</button></td><td><button type="button" className="mapping-delete" disabled={saving} onClick={() => deleteMapping(mapping)}>Hapus</button></td></tr>)}
            </tbody></table></div>
          )}
        </section>
